...
parser.add_argument('--modelload', '-n', type=str, required=True, help='Loading PyTorch model')
...
def main():
    ...
    if os.path.isfile(args.modelload):
        net_trained.load_state_dict({k.replace('module.',''):v for k,v in torch.load(args.modelload).items()})
    else:
