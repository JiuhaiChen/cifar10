...
palette_gaussian = sns.color_palette("bright", 1)
...
# Test setup
parser.add_argument('--test_bs', type=int, default=1200)
...
def get_anomaly_scores(net, loader):
    ...
    label = []
    with torch.no_grad():
        for x_tf_0, x_tf_90, x_tf_180, x_tf_270, x_tf_trans, target_trans_x, target_trans_y, target_class in tqdm(loader):
            '''
            scores.append(get_anomaly_score_batch(
                x_tf_0, x_tf_90, x_tf_180, x_tf_270, x_tf_trans, target_trans_x, target_trans_y, net
            ))
            '''
            r1, r2 = get_anomaly_score_batch(
                x_tf_0, x_tf_90, x_tf_180, x_tf_270, x_tf_trans, target_trans_x, target_trans_y, net
            )
            scores.append(r1)
            embedded.append(r2)
    ...
    return scores.cpu().numpy(), embedded, label.cpu().numpy()
...
def get_anomaly_score_batch(x_tf_0, x_tf_90, x_tf_180, x_tf_270, x_tf_trans, target_trans_x, target_trans_y, net):
    ...
    classification_loss = kl_div(class_uniform_dist, classification_smax)
    ...
    curr_score = classification_loss

    # t-sne
    # tsne = TSNE()
    # X_embedded_batch = tsne.fit_transform(penultimate[:batch_size].cpu().numpy())
    ...
    tsne = TSNE()
    X_embedded = tsne.fit_transform(penultimate[:4*batch_size].cpu().numpy())
    sns.scatterplot(X_embedded[:,0], X_embedded[:,1], hue=target_rots, legend='full', palette=palette)
    plt.show()
    exit()
    ...
    return curr_score, X_embedded_batch
...
def main():
    ...
    out_data = PerturbDataset(dset.CIFAR100('/home/jiuhai.chen/data/cifar100', train=False, download=True), train_mode=False)
    ...
    plt.scatter(X_embedded[:,0], X_embedded[:,1], c=in_probs, s=10, lw=0, cmap='RdYlGn')
    plt.colorbar()
    plt.show()
    exit()
