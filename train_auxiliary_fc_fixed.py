...
from models.wrn_prime import WideResNet, WideResNet_without_fc
...
# Checkpoints
parser.add_argument('--save', '-s', type=str, default='./snapshots/Fixed', help='Folder to save checkpoints.')
...
def main():
    ...
    train_data_in = dset.CIFAR10('/home/jiuhai.chen/data', train=True, download=True)
    test_data = dset.CIFAR10('/home/jiuhai.chen/data', train=False, download=True)
    num_classes = 10
    ...
    net_trained = WideResNet(args.layers, 10, args.widen_factor, dropRate=args.droprate)
    ...
    if os.path.isfile(args.model):
        net_trained.load_state_dict({k.replace('module.',''):v for k,v in torch.load(args.model).items()})
    else:
    ...
    # Create model
    net = WideResNet_without_fc(args.layers, fc_weight, fc_bias, num_classes, args.widen_factor, dropRate=args.droprate)
    ...
    for epoch in range(0, args.epochs):
        ...
        train(net, fc_weight, fc_bias, state, train_loader_in, optimizer, lr_scheduler)
...
def train(net, fc_weight, fc_bias, state, train_loader_in, optimizer, lr_scheduler):
...
def test(net, fc_weight, fc_bias, state, test_loader):
    loss_avg = 0.0
    net.eval()
    with torch.no_grad():
        correct = 0
